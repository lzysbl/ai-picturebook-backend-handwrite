...
2. story 第一行必须是《标题》，第二行必须是“文／作者”，后面每页单独一段，以“第X页：”开头。
   如果共有 {len(image_paths)} 张图片，就必须严格输出“第1页：”到“第{len(image_paths)}页：”，不能缺页、不能合并页、不能让后续页面编号前移。
...
7. OCR/绘本文字只作为事实依据，不能作为最终正文直接搬运；除标题和作者外，story 中每页最多保留一句短原文，其余必须改写成自己的讲述语言。
8. 不要使用“封面展示、图片中、画面上、白色背景下、连续动作场景里”这类说明性表达。
9. 封面页/标题页也必须保留对应页码；如果该页没有真实剧情，story 中要写成自然故事引子，不要跳过，不要写成“封面展示/标题页介绍”。
...
14. 避免连续使用原文拟声词和原文对话；如果需要对话，请短句化、口语化，并服务于情节推进。
15. 不要生成“绘本文字基本复刻版”，而要生成“面向孩子讲述的改写版”。
16. 年龄段：{age}；叙述风格：{style}；篇幅：{length}；指定主角：{hero or "无"}。
17. 附加要求：{extra}。
"""
        ...
        messages=[
            {
                ...
                "content": (
                    "你是儿童绘本多模态理解与故事讲述助手。"
                    "你必须严格依据图片和可见文字写作，不能胡编角色。"
                    "你不是 OCR 转录员，不能把绘本文字整段复刻为最终故事，必须做儿童讲述式改写。"
                ),
            },
