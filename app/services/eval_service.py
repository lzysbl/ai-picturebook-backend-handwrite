...
职责：
- 对生成故事进行自动指标评估，例如重复率、多样性和角色幻觉检测。
- 可选调用大模型作为评审，对故事完整性、贴图程度和儿童适配性打分。
- 输出论文实验和前端质量面板可使用的综合评价结果。

前端关联：
- `/ui/history`：故事详情页的质量评价、刷新评价、缓存读取。
- `/ui/generate`：生成故事完成后可同步计算基础质量指标。
- `/ui/camera`：实时识别保存故事后可计算质量评价。

主要路由：
- `app/routers/stories.py`：`/api/stories/{story_id}/quality`
- `app/routers/stories.py`：生成、保存、评价相关接口内部调用。
"""
